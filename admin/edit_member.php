...
if(isset($_GET['id'])){
    $query="select * from members where m_id=".$_GET['id'];
   $get_member= mysqli_query($conn,$query);
   $gm=mysqli_fetch_assoc($get_member);
   $query="select * from languages where m_id=".$_GET['id'];
   $lang= mysqli_query($conn,$query);
   $_SESSION['rec']=mysqli_num_rows($lang);
...
if(isset($_REQUEST['check'])&&$_REQUEST['check']=="checked"){
  // echo"check<br>";
...
if($_FILES['file']['size']!=0 && isset($_REQUEST['check']))
{
  
$query="update members set category='".$category."', f_name='".$f_name."',l_name='".$l_name."',designation='".$desig."',speciality='".$spec."',password='".$pass."',img_path='".$file_path."' where m_id=".$_GET['id'];

}
else if($_FILES['file']['size']!=0)
{
  $query="update members set category='".$category."', f_name='".$f_name."',l_name='".$l_name."',designation='".$desig."',speciality='".$spec."',img_path='".$file_path."' where m_id=".$_GET['id'];

}
else if(isset($_REQUEST['check'])){
  $query="update members set category='".$category."',f_name='".$f_name."',l_name='".$l_name."',designation='".$desig."',speciality='".$spec."',password='".$pass."' where m_id=".$_GET['id'];

}
else{
  $query="update members set category='".$category."',f_name='".$f_name."',l_name='".$l_name."',designation='".$desig."',speciality='".$spec."' where m_id=".$_GET['id'];
...
{
  if($_REQUEST['languages'][$i]!=''){
    $arr[]=$_REQUEST['languages'][$i];
    $count++;
  }
 }
}
...
// echo"count=". $count;
  ...
  {
   // echo $_SESSION['rec']."<br>";
//  print_r($_REQUEST['languages']);
    //  echo"come";
    
$query="select * from languages where m_id=".$_GET['id'];
$ids=mysqli_query($conn,$query);



    for($i=0;$i<$_SESSION['rec']-$count;$i++){
      $del=mysqli_fetch_assoc($ids);
      $query="delete from languages where l_id=".$del['l_id'];
   //  echo $query;
      ...
      // echo"enter";
         while($pick=mysqli_fetch_assoc($res))
         {
          ...
          //  if($_REQUEST['languages'][$i]!=''){
             ...
             $query="update languages set l_name='".$arr[$i++]."' where l_id=".$pick['l_id'];
             mysqli_query($conn,$query);
          //  }
          //  else{
          //    $i++;
  ...
  else
  {
     ...
     $diff=$count-$_SESSION['rec'];
    $gem=0;
    $query="select * from languages where m_id=".$_GET['id'];
    $pq=mysqli_query($conn,$query);
    
    while($pick=mysqli_fetch_assoc($pq)){
  $query="update languages set l_name='".$arr[$gem++]."' where l_id=".$pick["l_id"];
  mysqli_query($conn,$query);
 
    }
// echo "count=".count($arr);
// echo"gem=".$gem;
    for($i=$gem;$i<count($arr);$i++){
      // echo"running";
      $query="insert into languages (m_id,l_name) values(".$_GET['id'].",'".$arr[$i]."')";
      mysqli_query($conn,$query);
    }
    }
    $status="success";
...
<div class='col-md-9 align-self-start'>
    ...
    <div class='row shadow bg-body p-3 '>
        <div class='col-12 text-muted h3 fw-bold'>
          ...
          <div class='col-md-6'>
              ...
              <input type='text' class='form-control' value='<?php if(isset($gm['f_name']))echo $gm['f_name'];?>'required  name='f_name' data-validate-field='title' placeholder='enter your first name'> 

          </div>
          <div class='col-md-6'>
          <label class='form-label text-muted fw-bold'>last name</label>
              <input type='text' class='form-control'value='<?php if(isset($gm['l_name'])) echo $gm['l_name'];?>' required  name='l_name' data-validate-field='title' placeholder='enter your last name'> 
          ...
          <label class='form-label text-muted fw-bold'>Designation</label>
             <select  class='form-select' required name='designation' data-validate-field='category'>
               ...
               if(mysqli_num_rows($speciality)>0){
                while($sp=mysqli_fetch_assoc($speciality)){?>
                  <option value='<?php echo $sp['d_id'];?>'  <?php  if($gm['designation']==$sp['d_id']){ echo "selected";} ?>><?php echo $sp['d_name'];?></option> 
             ...
             <select  class='form-select' required name='specilality' >
               ...
               if(mysqli_num_rows($speciality)>0){
                while($sp=mysqli_fetch_assoc($speciality)){?>
                  <option value='<?php echo $sp['s_id'];?>'  <?php  if($gm['speciality']==$sp['s_id']){ echo "selected";} ?>><?php echo $sp['s_name'];?></option> 

                  <?php }}?>
                 <!-- <option value='Web Development' <?php  if($gm['speciality']=="Web Development"){ echo "selected";} ?>>Web development</option> -->
             </select>

             <label class='form-label text-muted fw-bold'>Category</label>
             <select  class='form-select' required name='category' >
               <option value=''>select</option>
               <option value='CR' <?php  if($gm['category']=="CR"){ echo "selected";} ?>>CR</option>
               <option value='Student' <?php  if($gm['category']=="Student"){ echo "selected";} ?>>Student</option>
             ...
             <label class='form-label text-muted fw-bold'>email</label>
              <input type='text' class='form-control'value='<?php if(isset($gm['email'])) echo $gm['email'];?>' disabled  name='email' data-validate-field='title' placeholder='enter your last name'> 
              <label class='form-label text-muted fw-bold'>password</label>
              <input type='text' class='form-control'value='<?php if(isset($gm['password']))echo $gm['password'];?>' disabled  name='pass' data-validate-field='title' placeholder='enter your last name'> 
              
         
              </div>
              <div class="form-check">
              <label class="form-check-label text-muted fw-bold" for="flexCheckDefault">
  ...
  <input class="form-check-input" type="checkbox" name='check' value="checked" id="flexCheckDefault">
           ...
           <label class='form-label ms-auto text-muted fw-bold lab1'>Upload image</label><br>
               <label class='form-label lab1' for='upload'>
                ...
                <input type='file'  class='file d-none' name='file'  id='upload'><br>
                <div class='error text-danger'>please uplaod file</div>
                <img src='<?php echo $gm['img_path']; ?>' class='img-thumbnail img_rem'  style='width:80%; height:400px'>
      ...
      <input type='text' class='form-control' id='keypress'>
           ...
           <div class='col-12 mt-2 '>
             <div class='row languages justify-content-start'>
                 <?php while( $fetch=mysqli_fetch_assoc($lang)){?>

                    <div class='col-md-4 mt-2 ' style='position:relative'><select name='languages[]' class='form-select' required>
                                             <option value=''>select</option>
                                             <?php $query="select * from technology";
                                             $tech=mysqli_query($conn,$query);
                                             while($tl=mysqli_fetch_assoc($tech)){ ?>
                                             <option <?php if($fetch['l_name']==$tl['t_id']){ echo "selected";}?> value='<?php echo $tl['t_id'];?>'><?php echo $tl['t_name'];?></option>
                                            <?php } ?>
                                             </select>
  ...
  <path d='M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708z'/>
           ...
           <div class='col-12 mt-2 text-end'>
                         <button  class='btn btn-outline-success ' name='add'>Edit</button>
                      </div>
