...
if(isset($_REQUEST['title'])){
    $f_name=$_REQUEST['title'];
   // $id=$_REQUEST['id'];
    //$limit=6*$id;
    
    $query="select * from members  where f_name='".$f_name."' order by m_id desc limit 3";
  ...
  .$row['speciality']."
    ...
    $query="select * from members where f_name='".$f_name."'  order by m_id desc ";
 ...
 if(mysqli_num_rows($limit)>3){
       ...
       $pag=ceil(mysqli_num_rows($limit)/3);
      for($i=0; $i<$pag;$i++){
          $g=$i;
          $g++;
   
   
   $output.=" <li class='page-item'><button class='page-link text-success search_index geek'   data-limit=". $i.">".$g."</button></li>";
    ...
    $output="<div class='text-center text-dark'>emtpy search</div>";
}
}
