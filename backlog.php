        ...
        <li class="nav-item dropdown d-block d-md-none ">
          ...
          <!-- <li><a class="dropdown-item text-light fw-bold  " href="home.php">Assign projects</a></li>
            ...
            <li><a class="dropdown-item" href="road_map.php?id=<?php echo $id;?>">Road Map</a></li>         
              ...
              <li><hr class="dropdown-divider text-light"></li>
              <li><a class="dropdown-item text-light fw-bold active" href="backlog.php?id=<?php echo $id;?>">Backlog</a></li>         
              <li><hr class="dropdown-divider text-light"></li>
...
<div class="collapse p-0" id="collapseExample">
  ...
  <form  method="POST" class='js-form module' enctype="multipart/form-data" id="fileUploadForm">
      ...
      <div class='col-md-2'>
       ...
       <div class='col-md-2'>
        <label class='form-label text-muted fs-5 fw-bold'>Start date</label>
       ...
       <div class='col-md-2'>
        <label class='form-label text-muted fs-5 fw-bold'>End date</label>
        <input type="text" name='e_date' data-validate-field='e_date' class='form-control' autocomplete='off' id="datepicker2">
          

       </div>
       <div class='col-md-2 mt-md-0 mt-1'>
       ...
       <input type="hidden" class='form-label' name="p_id"  value="<?php echo $pro['p_id']; ?>">
        ...
        <button  class='btn btn-outline-success mt-md-4'>create</button>
         ...
         <div class='col-12 '>
             ...
             <?php
             $query="select * from backlog where p_id=".$id;
             $version=mysqli_query($conn,$query);
             if(mysqli_num_rows($version)>0){
                 $i=0;
                 $var=3;
                 while($ver=mysqli_fetch_assoc($version)){
                     ?>
                    ...
                    <div class="accordion-item">
                      <h2 class="accordion-header " id="headingOne">
                        <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapse<?php echo$i; ?>" aria-expanded="true" aria-controls="collapseOne">
                         <div class='row'>
                             <div class='col-md-4'>
                                 <input type='text' name='sub'  class='form-control' value="<?php echo $ver['name'];?>" data-id="<?php echo$ver['b_id'];?>">
                             </div>
                             <div class='col-md-4'>
                             <input  name='s_date2' value="<?php echo date("Y-m-d ",strtotime($ver['s_date']));?>" class='form-control' data-id="<?php echo$ver['b_id'];?>" autocomplete='off' id="datepicker<?php echo++$var;?>">
      </div>

                             <div class='col-md-4'>
                             <input  name='e_date2' value="<?php echo  date("Y-m-d ",strtotime($ver['e_date'])); ?>" class='form-control' data-id="<?php echo $ver['b_id'];?>" autocomplete='off' id="datepicker<?php echo++$var;?>">
      </div>
                         </div>
                        </button>
                      </h2>
...
$s_date=$ver['s_date'];
$e_date=$ver['e_date'];
                        $query="select * from module where p_id=".$id." and (date>='".$s_date."' and date<='".$e_date."')";
                        $module=mysqli_query($conn,$query); 
                        if(mysqli_num_rows($module)>0){
                            while($mod=mysqli_fetch_assoc($module)){?>
                                <tr>
                                <td><?php echo $mod['subject'];?></td>
                                <td><?php echo $mod['status'];?>
  ...
  <script src="https://code.jquery.com/ui/1.13.0/jquery-ui.js"></script>
    ...
    <script>
        ...
        $(document).ready(function(){
           ...
           var length=<?php echo ($var>3)? $var:0;?>;
 ...
 var data = new FormData(form);
      ...
      data:data,
        ...
        if(response=="success"){
                      ...
                      $(".alert").html("Version Added Successfully");
                      setTimeout(function() {
  ...
  $(document).on("change","input[name='s_date2']",function(){
     // alert("hello");
      let s_date=$(this).val();
      let e_date=$(this).parent().parent().find("input[name='e_date2']").val();
      let name=$(this).parent().parent().find("input[name='sub']").val();
      let id=$(this).data("id");
...
$.ajax({
  method:"POST",
  url:"php/update_backlog.php",
  data:{
    id:id,name:name,s_date:s_date,e_date:e_date
  },
  success:function(data){
    if(data=="success"){
         ...
         $(".alert").removeClass("d-none");
         $(".alert").addClass("d-block");
         $(".alert").html("Version Updated Successfully");
         setTimeout(function() {
           $(".alert").removeClass("d-block");
         $(".alert").addClass("d-none");
    ...
    $(document).on("change","input[name='e_date2']",function(){
     // alert("hello");
      let e_date=$(this).val();
      let s_date=$(this).parent().parent().find("input[name='s_date2']").val();
      let name=$(this).parent().parent().find("input[name='sub']").val();
      let id=$(this).data("id");
...
$.ajax({
  ...
  success:function(data){
    ...
    $(document).on("blur","input[name='sub']",function(){
     // alert("hello");
      let name=$(this).val();
      let e_date=$(this).parent().parent().find("input[name='e_date2']").val();
      let s_date=$(this).parent().parent().find("input[name='s_date2']").val();
