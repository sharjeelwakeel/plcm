...
if(isset($_REQUEST['id'])){
 ...
 $query="select * from projects where p_id=".$id;
  ...
  $pro=mysqli_fetch_assoc($project);
    ...
    //assign member
    $query="select members.m_id as m_id,f_name from assign_projects,members where p_id=".$id." and members.m_id=assign_projects.m_id";
    //echo $query;
...
$query="select * from cost where c_id=".$c_id;
  $cost=mysqli_query($conn,$query);
  $fet=mysqli_fetch_assoc($cost);
...
<div class='row bg-body p-3 h-auto '>
      ...
      <div class='col-md-3'>
        ...
        <input class='form-control' placeholder='Name' value='<?php echo $fet['name']?>' name='name' data-validate-field='subject'>

       </div>

       <div class='col-md-3 '>
        <label class='form-label text-muted fw-bold fs-5'>price</label>
        <input class='form-control' type='number' placeholder='price' value='<?php echo $fet['price']?>' name='price' data-validate-field='price'>


       </div>
       
       <div class='col-md-6 '>
       <label class='form-label text-muted fw-bold fs-5'>desription</label>
        <input class='form-control'placeholder='description' value='<?php echo $fet['description']?>' name='description' data-validate-field='description'>
         
      </div>

      <div class='col-md-3'>
        <label class='form-label text-muted fw-bold fs-5'>Collector</label>
  ...
  <option  value="<?php echo $asgn['m_id']; ?>" <?php echo ($fet['m_id']==$asgn['m_id'])?"selected":'';?>><?php echo $asgn['f_name']; ?></option>
<?php }?>
  ...
  <div class='col-md-3'>
        ...
        <select class="form-select text-muted" aria-label="Default select example" name='module' data-validate-field='module'>
  <option selected value=''>select</option>
  <?php while($mod=mysqli_fetch_assoc($module)){?>
  <option value="<?php echo $mod['mod_id']; ?>" <?php echo ($mod['mod_id']==$fet['mod_id'])?"selected":'';?> ><?php echo $mod['subject']; ?></option>
<?php }?>
       ...
       <div class='col-md-6 mt-md-0 mt-2 text-end'>
       <label class='form-label text-muted fw-bold fs-5 invisible'>Priority</label>
    
       <input type="hidden" class='form-label' name="c_id"  value="<?php echo $fet['c_id']; ?>">
       
        <button name='module' class='btn btn-outline-success mt-md-4'>update</button>
     ...
     data:data,
       ...
       if(response=="success"){
        ...
        $(".alert").removeClass("d-none");
                      $(".alert").first().html("Cost Update Successfully");
                      $(".alert").addClass("d-block");
                      setTimeout(function() {
                        $(".alert").removeClass("d-block");
                      $(".alert").addClass("d-none");
                      
                      window.location.replace("add_cost.php?id="+<?php echo $fet['p_id'];?>);
                      }, 3000);
       }
