...
$c_id=14;
$query="insert into notifications (p_id,c_id,u_id,name,u_category,description) values(".$p_id.",".$m_id.",".$c_id.",' '".",'admin','delete module ".$subject." in')";
mysqli_query($conn,$query);
if(mysqli_num_rows($notify_assign)>0){
while($fetch=mysqli_fetch_assoc($notify_assign))
{
$c_id=$fetch['m_id'];
$query="insert into notifications (p_id,c_id,u_id,name,u_category,description) values(".$p_id.",".$m_id.",".$c_id.",' '".",'member','delete module ".$subject." in')";
mysqli_query($conn,$query);
