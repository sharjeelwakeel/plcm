...
<style>
   .blurred, .findInListDiv,.checklistContainer,.checklist{
    width:100%!important;
}
        ...
        <li class="nav-item dropdown d-block d-md-none ">
          ...
          <!-- <li><a class="dropdown-item text-light fw-bold  " href="home.php">Assign projects</a></li>
            ...
            <li><a class="dropdown-item" href="road_map.php?id=<?php echo $id;?>">Road Map</a></li>         
              ...
              <li><hr class="dropdown-divider text-light"></li>
              <li><a class="dropdown-item text-light fw-bold active" href="quality.php?id=<?php echo $id;?>">Quality</a></li>         
              <li><hr class="dropdown-divider text-light"></li>
...
<div class='row bg-body p-3 h-auto '>
          ...
          <div class='col-12'>
          <p class='text-end'>
          <?php   $query="select * from permission_status where m_id=".$m_id." and p_id=15";
...
// exit;
?>
  <a class="btn btn-success <?php echo($cfg['p_id']==15 && $cfg['status']=="no")?"disabled":"" ?> " data-bs-toggle="collapse" href="#collapseExample" role="button" aria-expanded="false" aria-controls="collapseExample">
...
<div class="collapse p-0" id="collapseExample">
  ...
  <form  method="POST" class='js-form module' enctype="multipart/form-data" id="fileUploadForm">
      ...
      <div class='col-md-12'>
       ...
       <label class='form-label text-muted fw-bold fs-5'>Test Purpose</label>
      
         <select name='req' class='form-select text-muted' data-validate-field='req'>
           <option value=''>select</option>
           <?php if(mysqli_num_rows($require)>0){
             while($req=mysqli_fetch_assoc($require)){
             ?>
             <option value="<?php echo $req['r_id'];?>"><?php echo $req['name'];?></option>
         <?php  }}?>
         </select>
      ...
      <div class='col-md-3'>
        <label class='form-label text-muted fs-5 fw-bold'>Test case</label>
        <input class='form-control'placeholder='Test Case Name' name='subject' data-validate-field='subject'>

       </div>

       <div class='col-md-6'>
        <label class='form-label text-muted fw-bold fs-5'>Description</label>
        <input class='form-control'placeholder='Test Case Name' name='description' data-validate-field='description'>

       </div>

       <div class='col-md-3'>
        <label class='form-label text-muted fw-bold fs-5'>Assigned</label>
        <select class="form-select text-muted" aria-label="Default select example" name='assigned' data-validate-field='assigned'>
  <option selected value=''>Select</option>
  <?php while($asgn=mysqli_fetch_assoc($assign)){?>
  <option value="<?php echo $asgn['m_id']; ?>"><?php echo $asgn['f_name']; ?></option>
<?php }?>
       ...
       <label class='form-label text-muted fw-bold fs-5'>Browsers/Devices</label>
							<div class="input-group">
     
        <?php $query="select * from Browser"; 
       $browser =mysqli_query($conn,$query);?>
        <select name="projects[]" class='skills' multiple="multiple" id="skills">
		                             <?php if (mysqli_num_rows($browser)>0){
                                        while($pro=mysqli_fetch_assoc($browser)){ ?>

                                                 <option value="<?php echo $pro['b_id'];?>"><?php echo $pro['name']?></option>
                                        <?php }  }?>
		                        </select>
       ...
       <div class='col-md-6'>
       <div class="form-group mb-3">
       <label class='form-label text-muted fw-bold fs-5'>CPU</label>
							<div class="input-group">
     
        <?php $query="select * from cpu"; 
       $browser =mysqli_query($conn,$query);?>
        <select name="projects[]" multiple="multiple" class='skills' id="cpu">
		                             <?php if (mysqli_num_rows($browser)>0){
                                        while($pro=mysqli_fetch_assoc($browser)){ ?>

                                                 <option value="<?php echo $pro['c_id'];?>"><?php echo $pro['name']?></option>
                                        <?php }  }?>
		                        </select>
       ...
       <div class='col-md-6'>
       <div class="form-group mb-3">
       <label class='form-label text-muted fw-bold fs-5'>Operating System</label>
							<div class="input-group">
     
        <?php $query="select * from operating_system"; 
       $browser =mysqli_query($conn,$query);?>
        <select name="projects[]" multiple="multiple" class='skills' id="os">
		                             <?php if (mysqli_num_rows($browser)>0){
                                        while($pro=mysqli_fetch_assoc($browser)){ ?>

                                                 <option value="<?php echo $pro['o_id'];?>"><?php echo $pro['name']?></option>
                                        <?php }  }?>
		                        </select>
       ...
       <div class='col-md-6'>
       <div class="form-group mb-3">
       <label class='form-label text-muted fw-bold fs-5'>Databases</label>
							<div class="input-group">
     
        <?php $query="select * from storage"; 
       $browser =mysqli_query($conn,$query);?>
        <select name="projects[]" multiple="multiple" class='skills' id="database">
		                             <?php if (mysqli_num_rows($browser)>0){
                                        while($pro=mysqli_fetch_assoc($browser)){ ?>

                                                 <option value="<?php echo $pro['d_id'];?>"><?php echo $pro['name']?></option>
                                        <?php }  }?>
		                        </select>
       ...
       <div class='col-md-6'>
        <label class='form-label text-muted fw-bold fs-5'>Due Date</label>
        <input type="text" name='s_date' data-validate-field='s_date' class='form-control' autocomplete='off' id="datepicker">
       
       </div>  
       <div class='col-md-6'>
        <label class='form-label text-muted fw-bold fs-5'>Staus</label>
...
<table id="userTable" class=' table table-hover '>
        ...
        <tbody>
           ...
           <tr>
             <td ><a href='test_detail.php?id=<?php echo $id;?>&&t_id=<?php echo $fet['t_id'];?>' class='text-decoration-none text-dark'><?php echo $fet['t_id'];?></a></td>
             <td><?php echo $fet['name'];?></td>
             <td><?php echo $fet['req_name'];?></td>
             <td><?php echo $fet['status'];?></td>
             
             <td><?php echo $fet['f_name'];?></td>
             <td><?php echo date("Y-m-d ",strtotime($fet['due']));;?></td>
             <td>
             <?php   $query="select * from permission_status where m_id=".$m_id." and p_id=16";
...
?>
             ...
             <a class="btn btn-success btn-sm d-none <?php echo($cfg['p_id']==16 && $cfg['status']=="no")?"disabled":"" ?>" data-bs-toggle="tooltip" data-bs-html="true" title="<em >update</em> " href="edit_test.php?id=<?php echo $id."&& t_id=".$fet['t_id'];?>"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
  <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
  <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z"/>
</svg></a>
<?php   $query="select * from permission_status where m_id=".$m_id." and p_id=17";
...
// exit;
?>
                          <button class="btn btn-success btn-sm delete  <?php echo($cfg['p_id']==17 && $cfg['status']=="no")?"disabled":"" ?>" data-bs-toggle="tooltip" data-bs-html="true" title="<em>delete</em>" data-id="<?php echo$fet['t_id'];?>" ><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-archive-fill" viewBox="0 0 16 16">
  <path d="M12.643 15C13.979 15 15 13.845 15 12.5V5H1v7.5C1 13.845 2.021 15 3.357 15h9.286zM5.5 7h5a.5.5 0 0 1 0 1h-5a.5.5 0 0 1 0-1zM.8 1a.8.8 0 0 0-.8.8V3a.8.8 0 0 0 .8.8h14.4A.8.8 0 0 0 16 3V1.8a.8.8 0 0 0-.8-.8H.8z"/>
</svg></button>



<a class="btn btn-success btn-sm" data-bs-toggle="tooltip" data-bs-html="true" title="<em >defects</em> "  href="defect.php?id=<?php echo $id."&& t_id=".$fet['t_id'];?>">
 <img src="admin/web_material/defect.png" style="width:16px;height:16px">
</a>
  ...
  <div class='text-center'>
    Test Case Added Successfully    </div>
</div>
